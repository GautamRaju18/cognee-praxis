...
def test_create_and_get_decision(clean_cognee_test_dataset):
    with TestClient(app) as client:
        ...
        listed = client.get("/decisions", params={"topic": "hiring"}).json()
        assert any(d["id"] == body["id"] for d in listed)

        # GET by id
        got = client.get(f"/decisions/{body['id']}")
        assert got.status_code == 200
        assert got.json()["title"] == PAYLOAD["title"]

        # 404 for unknown id
        assert client.get(f"/decisions/{uuid.uuid4().hex}").status_code == 404


def test_decision_reached_cognee_graph(clean_cognee_test_dataset):
    ...
    nodes, _edges = asyncio.run(cognee_service.get_graph_data())
